...
public class SystemSession implements LocalSession<EmployeeUserDTO>, Serializable {
     ...
     */
    public void getWarnings(JDBConnection connection) {
        try {
            ...
            if (AppConfig.isAutoPay(connection)) {
     ...
     * @throws SQLException
     */
    public void systemValid(JDBConnection connection) throws SystemException, SQLException {
        try {
            ...
            if (AppConfig.getParameterBoolean(connection, "VALIDA_ADMINISTRACIO") && Func.isNull(current_administration)) {
    ...
    public boolean isLock(JDBConnection connection) throws SQLException {
        SessionDAO dao = new SessionDAO(false, "SystemSession");
        boolean currentStatus = dao.isSessionClose(connection, current_session);
